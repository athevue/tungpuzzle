// Keep track of moves and time, will reflect after solving puzzle
var moves = 0;
var time = 0;
...
// Swap the classes of two cells to move tiles around
...
// Shuffle all tiles randomly on the board
function shuffle() {
    moves = 0;
    time = 0;
    ...
    // Loop through every cell and swap it with a random one
...
// Simple game starts with just last two tiles swapped
function simpleGame() {
    swapTiles("cell44", "cell43"); // swap last two tiles
...
// Handle clicking a tile to move it if adjacent to empty spot
function clickTile(row, column) {
    var tile = document.getElementById("cell" + row + column).className;

    if (tile != "tile16") { // skip the empty tile
        // Check RIGHT
        if (column < 4 && document.getElementById("cell" + row + (column + 1)).className == "tile16") {
            swapTiles("cell" + row + column, "cell" + row + (column + 1));
            moves++;
            document.getElementById("moves").innerHTML = moves;
            setTimeout(() => { Win() }, 200); // check if player won and delay mechanism to prevent multiple alerts
            return;
        }
